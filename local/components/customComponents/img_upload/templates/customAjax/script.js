...
$(document).ready(function () {
    ...
    $("#f_send").on("click", function () {
        ...
        jQuery.ajax({
            url: '/local/components/customComponents/img_upload/templates/customAjax/ajax/uploadImg.php',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            ...
            success: function (response) {
                ...
                if ($(response).filter('p').hasClass('uplOk')) {
                 $("#f_contact").remove();
                 $("#feedback :header").remove();
                    $("#feedback").before($(response).filter('p'));
                }
